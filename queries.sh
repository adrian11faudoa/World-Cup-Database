...
echo "$($PSQL "SELECT GREATEST(
    (SELECT MAX(winner_goals) FROM games),
    (SELECT MAX(opponent_goals) FROM games)) AS overall_max;")"
...
echo "$($PSQL "SELECT DISTINCT name FROM teams RIGHT JOIN games ON teams.team_id IN (games.winner_id, games.opponent_id) WHERE round = 'Eighth-Final' AND year=2014 ORDER BY name")"
